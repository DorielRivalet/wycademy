...
<script lang="ts">
	...
	const consoleMessages = {
		...
		nosnakeEnergy: 'Ran out of snakeEnergy! 💨',
		missingsnakeEnergy: 'Not enough snakeEnergy! 💨',
		toasty: 'Toasty! 🍞',
		paralysis: [
			'Bzzz! ⚡',
			'Zap! ⚡',
			'This snake is high voltage! ⚡',
			'Used a lot of snakeEnergy! ⚡',
			'Snake microwave! ⚡',
	...
	const SOUNDS = {
		...
		snakeEnergyLoss: 'assets/sound/mixkit-tech-break-fail-2947.wav',
	...
	// 				maxSpeed - startingSpeed,
	// 			);
	// 			snakeEnergyElement.max += HUNTER_TYPES.black.snakeEnergyMaxGain;
	// 			snakeEnergy += HUNTER_TYPES.black.snakeEnergyGain;
	// 			snakeEnergy = Math.max(snakeEnergy, 0);
	// 			hunters.forEach((hunter) => {
	// 				if (hunter) {
	// 					hunter.age++;
	// 				}
	...
	// 				maxSpeed - startingSpeed,
	// 			);
	// 			snakeEnergyElement.max += HUNTER_TYPES.yellow.snakeEnergyMaxGain;
	// 			snakeEnergy += HUNTER_TYPES.yellow.snakeEnergyGain;
	// 			snakeEnergy = Math.max(snakeEnergy, snakeEnergyElement.max);
	// 			hunters.forEach((hunter) => {
	// 				if (hunter) {
	// 					hunter.age =
	...
	// 			);
	// 			sendMessageToConsole(
	// 				`You can now fire ${snakeEnergyElement.max} fireballs! 🔥`,
	// 			);
	...
	// 				maxSpeed - startingSpeed,
	// 			);
	// 			snakeEnergyElement.max += HUNTER_TYPES.purple.snakeEnergyMaxGain;
	// 			snakeEnergy += HUNTER_TYPES.purple.snakeEnergyGain;
	// 			snakeEnergy = Math.max(snakeEnergy, 0);
	// 			hunters.forEach((hunter) => {
	// 				if (hunter) {
	// 					hunter.age += 5;
	...
	// 				}
	// 			});
	// 			snakeEnergyElement.max += HUNTER_TYPES.orange.snakeEnergyMaxGain;
	// 			snakeEnergy += HUNTER_TYPES.orange.snakeEnergyGain;
	// 			snakeEnergy = Math.max(snakeEnergy, snakeEnergyElement.max);
 ...
 @see https://stackoverflow.com/a/1527820/2124254 */
	...
	// 		if (rgba2hex(borderColor) === COLORS.border) {
	...
	// 	snakeEnergyElement.max = startingsnakeEnergy;
	// 	score = 0;
	// 	turns = 0;
	// 	toastyCount = 0;
	...
	// 		if (
	// 			rgba2hex(borderColor) !== fireballColors.hit &&
	// 			(fireball.x < 0 ||
	...
	// 			if (snake.canBurnBlueBorder && rgba2hex(borderColor) === COLORS.blue) {
	...
	// 			}
	// 		} else if (rgba2hex(borderColor) === fireballColors.hit) {
	...
	// function toasty() {
	// 	toastyCount = 0;
	// 	snakeEnergy = snakeEnergyElement.max;
	...
	// 		rgba2hex(borderColor) === COLORS.blue
	// 	) {
	...
	// 	if (hitEdge && rgba2hex(borderColor) === fireballColors.hit) {
	// 		snake.extraSpeed = Math.min(
	// 			snake.extraSpeed + 1,
	// 			maxSpeed - startingSpeed,
	// 		);
	...
	// 		snake.firing = true;
	// 		if (snakeEnergy <= 0) {
	// 			sendMessageToConsole(consoleMessages.nosnakeEnergy);
	// 			return;
	// 		}
	...
	// 			sendMessageToConsole(consoleMessages.nosnakeEnergy);
	// 			return;
	// 		} else if (snakeEnergy < SNAKE_ACTIONS.ringOfFire.cost) {
	// 			sendMessageToConsole(consoleMessages.missingsnakeEnergy);
	// 			return;
	// 		} else if (
	// 			snake.canUseUltimate &&
	// 			!drewRingOfFire &&
	// 			snakeEnergy >= SNAKE_ACTIONS.ringOfFire.cost &&
	// 			ringOfFire.length === 0
	// 		) {
	// 			snakeEnergy -= SNAKE_ACTIONS.ringOfFire.cost;
	// 			snake.poisoned = false;
	...
	// 			return;
	// 		}
	// 		if (snake.paralysis || snakeEnergy <= 0) {
	// 			sendMessageToConsole(consoleMessages.nosnakeEnergy);
	// 			return;
	// 		} else if (snakeEnergy < SNAKE_ACTIONS.paralysis.cost) {
	// 			sendMessageToConsole(consoleMessages.missingsnakeEnergy);
	// 		} else if (
	// 			!snake.paralysis &&
	// 			!drewParalysis &&
	// 			snakeEnergy >= SNAKE_ACTIONS.paralysis.cost
	// 		) {
	// 			snake.poisoned = false;
	...
	// 			startingText();
	// 			drawHitHunters();
	// 			snakeEnergy = startingsnakeEnergy;
	// 			requestAnimationFrame(loop);
	// 			gameState = GAME_STATES.RUNNING;
	// 			gameStateText = 'Pause';
	// 			borderColor = COLORS.border;
	...
	$: gameButtonClass = '' === 'Paused' ? 'paused' : 'running';
	...
	<div class="game-container">
		<div class="left">
			<Button
				icon={Notification}
				kind="tertiary"
				on:click={() => console.log('')}>Download History Logs</Button
			>
		...
		<div class="right">
			...
			<progress id="snakeEnergy" max="10" value="10"></progress>

			<div class="game-values">
				<div id="timer">{0}</div>
				<div id="score">{score}</div>
				<div id="speed">{0}</div>
				<div id="snake-length">{0}</div>
				<div class="hit-hunters">
					<div id="eaten">{0}</div>
					<div id="roasted">{0}</div>
					<div id="zapped">{0}</div>
					<div id="toasty">{0}</div>
				</div>
			</div>
